// Copy of csrc/libtorch_stable/gdn/fused_gdn_decode_kernel.cu with an early PDL
// trigger (VLLM_PDL_GEMV). Built at runtime by pdl_ext/__init__.py.
/*
...
__global__ __launch_bounds__(kThreads, 2) void gdn_decode_post_conv_mtp_kernel(
  ...
  // Flash-Next PDL: let the next kernel (out_proj's PDL GEMV) start streaming
  // its weights now; it still waits (griddepcontrol.wait) for our completion
  // before reading our output.
